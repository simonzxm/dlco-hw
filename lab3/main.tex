...
\begin{figure}[H]
    \centering
    \subcaptionbox{原理图}{\includegraphics[width=0.5\textwidth]{assets/exp1/sch.png}}
...
\begin{figure}[H]
    ...
    \subcaptionbox{电路图}{\includegraphics[width=0.45\textwidth]{assets/exp2/circuit.png}}
    \caption{4位移位寄存器电路设计}
\end{figure}
...
\begin{figure}[H]
    ...
    \subcaptionbox{电路图}{\includegraphics[width=0.9\textwidth]{assets/exp3/circuit.png}}
...
\begin{figure}[H]
    \centering
    \subcaptionbox*{(b)}{\includegraphics[width=0.3\textwidth]{assets/exp5/circuit2.png}}
    \subcaptionbox*{(c)}{\includegraphics[width=0.65\textwidth]{assets/exp5/circuit3.png}}
    \caption{数字时钟电路图}
\end{figure}
...
\begin{figure}[H]
    \centering
    \subcaptionbox{错误的电路图}{\includegraphics[width=0.4\textwidth]{assets/exp5/error.png}}
    \subcaptionbox{正确的电路图}{\includegraphics[width=0.4\textwidth]{assets/exp5/circuit2.png}}
    \caption{数字时钟实验中出现的错误}
\end{figure}

\section{思考题}

\subsection{异步清零和异步置数的 4 位计数器}

Logisim 中的 0,1 引脚可以直接用来实现异步清零和异步置数的功能。
我们添加引脚 AsyncCLR 和 AsyncLD，电路图如下

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/p1.png}
    \caption{异步清零和异步置数的 4 位计数器电路图}
\end{figure}
...
让计数器在 6-15 之间循环，即可实现。具体来说，当计数器达到最大值 15 时，
RCO 会输出高电平，可以将其连回 LD，重新加载初始值 6 即可。电路图如下
...
\begin{figure}[H]
    ...
    \begin{circuitikz}
        \node[
            flipflop JK,
            flipflop def={t1=D, t3=En, t4=RCO, tu=LD}
        ] (cnt) at (0,0) {};
        \node[left] at (cnt.pin 1) {0110};
        \node[left] at (cnt.pin 3) {1};
        \draw (cnt.pin 4) -- ++(0.3,0) node[circ]{} |- (cnt.up);
        \draw (cnt.pin 4) -- ++(1,0) node[right]{RCO};
        \draw (cnt.pin 6) -- ++(1,0) node[right]{Q};
        \draw (cnt.pin 2) -- ++(-1,0) node[left]{CLK};
    \end{circuitikz}
    \caption{模10计数器电路图}
\end{figure}

\subsection{8位二进制伪随机序列生成器}

用两个 4 位移位寄存器级联起来，利用 LFSR 即可实现，逻辑如下

\begin{lstlisting}[language=Verilog, caption={8位二进制伪随机序列生成器原理}]
module lfsr (
    input [7:0] seed,
    input clk,
    input load,
  ...
  always @(posedge clk) begin
    if (load) dout <= seed;
    else dout <= {dout[7] ^ dout[5] ^ dout[4] ^ dout[3], dout[7:1]};
  end

endmodule
\end{lstlisting}

加载种子 0x08，得到序列 08 11 23 47 8E 1C 38 71 E2 C4 89 12 25 4B 97 2E 5C B8 70 E0 C0 81 03 06 0C 19 32 64 C9 92 24 49 93 26 4D 9B 37 6E DC B9 72 E4 C8 90 20 41 82 05 0A 15 2B 56 AD 5B B6 6D DA B5 6B D6 AC 59 B2 65 CB 96 2C 58 B0 61 C3 87 0F 1F 3E 7D FB F6 ED DB B7 6F DE BD 7A F5 EB D7 AE 5D BA 74 E8 D1 A2 44 88 10 21 43 86 0D 1B 36 6C D8 B1 63 C7 8F 1E 3C 79 F3 E7 CE 9C 39 73 E6 CC 98 31 62 C5 8B 16 2D 5A B4 69 D2 A4 48 91 22 45 8A 14 29 52 A5 4A 95 2A 54 A9 53 A7 4E 9D 3B 77 EE DD BB 76 EC D9 B3 67 CF 9E 3D 7B F7 EF DF BF 7E FD FA F4 E9 D3 A6 4C 99 33 66 CD 9A 35 6A D4 A8 51 A3 46 8C 18 30 60 C1 83 07 0E 1D 3A 75 EA D5 AA 55 AB 57 AF 5F BE 7C F9 F2 E5 CA 94 28 50 A1 42 84 09 13 27 4F 9F 3F 7F FF FE FC F8 F0 E1 C2 85 0B 17 2F 5E BC 78 F1 E3 C6 8D 1A 34 68 D0 A0 40 80 01 02 04

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/p3.png}
    \caption{8位二进制伪随机序列生成器电路图}
\end{figure}

\subsection{支持写后读的寄存器堆}

只需要对于输出 RDA 和 RDB 进行特殊处理即可，原理如下。

\begin{lstlisting}[language=Verilog, caption={支持写后读的寄存器堆原理}]
module regfile (
    input  wire        clk,
    input  wire        WE,
    input  wire [ 4:0] RW,
  ...
  assign RDA = (WE && (RW == RA)) ? Din : registers[RA];
  assign RDB = (WE && (RW == RB)) ? Din : registers[RB];
...
\begin{figure}[H]
    ...
    \begin{circuitikz}
        \node[muxdemux,
            muxdemux def={Lh=3, Rh=1, NL=2, NB=1},
        ...
        \node[left] at (muxa.lpin 1) {RDA};
        \node[left] at (muxb.lpin 1) {RDB};
        \node[right] at (muxa.rpin 1) {RDA\_RAW};
        \node[right] at (muxb.rpin 1) {RDB\_RAW};
        \node[left] at (muxa.lpin 2) {Din};
        \node[left] at (muxb.lpin 2) {Din};
        \node[below] at (muxa.bpin 1) {WE \&\& (RW == RA)};
        \node[below] at (muxb.bpin 1) {WE \&\& (RW == RB)};
    \end{circuitikz}
    \caption{支持写后读的寄存器堆电路图}
\end{figure}
